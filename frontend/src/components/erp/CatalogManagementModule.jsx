...
import {
  ...
  ArrowRightLeft, Download, Upload, Tag, Star, Eye, Coins,
} from 'lucide-react';
...
export default function CatalogManagementModule({ token }) {
  ...
  // RC-UI-03: client-side pagination for catalog items (10/hal)
  const itemsPagination = useClientPagination(items, 10);
  ...
  const handleRefreshItemHpp = async (item) => {
    if (!selectedCatalog) return;
    setRefreshingItemId(item.id);
    try {
      const r = await cat(`/${selectedCatalog.id}/items/${item.id}/refresh-hpp`, { method: 'POST', headers });
      const d = await r.json();
      if (!r.ok) {
        toast.error(d.detail || 'Item belum tertaut ke Model/FG dari RnD — HPP tidak bisa di-refresh.');
        return;
      }
      toast.success(`HPP "${item.name}" diperbarui: ${fmtRp(d.hpp)}`);
      loadItems(selectedCatalog.id, searchQ, filterStatus);
    } catch (e) {
      toast.error('Gagal refresh HPP item');
    } finally { setRefreshingItemId(null); }
  };

  // Fase 3b — Sinkron stok item Toko dari FG (by SKU varian / material). Auto-override.
  const handleSyncItemFgStock = async (item) => {
    if (!selectedCatalog) return;
    setSyncingItemId(item.id);
    try {
      const r = await cat(`/${selectedCatalog.id}/items/${item.id}/sync-fg-stock`, { method: 'PUT', headers });
      const d = await r.json();
      if (!r.ok) {
        toast.error(d.detail || 'Gagal sinkron stok FG — pastikan master FG (SKU sama) sudah ada.');
        return;
      }
      toast.success(
        `Stok "${item.name}" disinkron dari FG: ${Number(d.stock_quantity).toLocaleString('id-ID')} pcs `
        + `(on-hand ${Number(d.fg_onhand).toLocaleString('id-ID')} − reserve ${Number(d.fg_reserved).toLocaleString('id-ID')})`
      );
      loadItems(selectedCatalog.id, searchQ, filterStatus);
      loadCatalogs();
    } catch (e) {
      toast.error('Gagal sinkron stok FG item');
              ...
              <div className="flex items-center gap-2 flex-wrap justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={handleRefreshHppBulk} disabled={refreshingHpp}
                    title="Tarik-ulang HPP semua item dari RnD (Model/FG)" data-testid="catalog-refresh-hpp-bulk">
                ...
                <div className="rounded-xl border overflow-hidden border-[var(--glass-border)] bg-[var(--card-surface)] shadow-[var(--shadow-card)]">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {itemsPagination.paged.map(item => {
                          ...
                          <tr key={item.id} className="hover:bg-foreground/5 transition-colors">
                            <td className="px-4 py-3">
                              ...
                              <div className="text-xs text-muted-foreground flex items-center gap-2 flex-wrap">
                                ...
                                {item.variant_info && <span className="text-indigo-600 dark:text-indigo-300">· {item.variant_info}</span>}
                                {item.variant_sku && (
                                  <span
                                    className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-mono text-[10px]"
                                    title="Tertaut ke varian produksi / Finished Goods (by SKU)"
                                    data-testid="catalog-item-variant-sku">
                                    <Layers className="w-3 h-3" /> FG:{item.variant_sku}
                                  </span>
                                )}
                              </div>
                            </td>
                            <td className="px-4 py-3 text-center" data-testid="catalog-item-price-cell">
                              {(() => {
                                const jual = item.harga_jual ?? item.price ?? 0;
                                const coret = item.harga_coret ?? item.original_price ?? 0;
                                const hpp = item.hpp ?? 0;
                                return (
                                  <>
                                    {coret > jual && (
                                      <div className="text-xs text-muted-foreground line-through">{fmtRp(coret)}</div>
                                    )}
                                    <div className="font-semibold text-emerald-600 dark:text-emerald-400">{fmtRp(jual)}</div>
                                    {hpp > 0 && (
                                      <div className="text-[10px] text-muted-foreground" title="Harga Pokok Produksi (dari RnD)">
                                        HPP {fmtRp(hpp)}
                                      </div>
                                    )}
                                  </>
                            ...
                            <td className="px-4 py-3 text-center">
                              ...
                              {item.variant_sku && item.fg_available != null && (
                                <div className="text-[10px] text-indigo-600 dark:text-indigo-300 mt-0.5" title="Stok Finished Goods tersedia (dari sinkron terakhir)">
                                  FG: {Number(item.fg_available).toLocaleString('id-ID')} tersedia
                                </div>
                              )}
                            </td>
                            <td className="px-4 py-3 text-center">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-center gap-1">
                                ...
                                {(item.variant_sku || item.fg_material_id) && (
                                  <Button size="icon" variant="ghost" className="h-7 w-7"
                                    title="Sinkron Stok dari Finished Goods (by SKU varian)"
                                    disabled={syncingItemId === item.id}
                                    data-testid="catalog-item-sync-fg"
                                    onClick={() => handleSyncItemFgStock(item)}>
                                ...
                                <Button size="icon" variant="ghost" className="h-7 w-7"
                                  title="Refresh HPP dari RnD (Model/FG)"
                                  disabled={refreshingItemId === item.id}
                                  data-testid="catalog-item-refresh-hpp"
                                  onClick={() => handleRefreshItemHpp(item)}>
