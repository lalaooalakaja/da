...
export default function ProductionCMTBillingModule({ portalId, userRole }) {
  ...
  }, [businessType, status, search]);

  useEffect(() => { load(); }, [businessType, status]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="space-y-4" data-testid="prod-cmt-billing">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <SmartNativeSelect
          ...
          className="h-9 border border-border rounded-lg px-2 text-sm"
        ...
        <span className="ml-auto text-xs text-muted-foreground">{rows.length} tagihan</span>
      </div>
            ...
            <tbody className="divide-y divide-border/50">
              {loading ? (
                ...
                <tr><td colSpan={9} className="text-center py-12 text-muted-foreground" data-testid="cmt-billing-empty">
                  ...
                  <p className="font-medium">Belum ada tagihan CMT</p>
                ...
                <tr key={r.id} className="hover:bg-muted/40" data-testid={`cmt-billing-row-${r.payment_code}`}>
                  ...
                  <td className="px-3 py-2.5">{r.cmt_name || '-'}</td>
