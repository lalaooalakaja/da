  ...
  const load = useCallback(async () => {
    ...
    try {
      const [list, sm] = await Promise.all([
        api('GET', `/api/wms/quarantine?status=${status}`, token),
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <SmartNativeSelect value={status} onChange={(e) => setStatus(e.target.value)}
          className="border border-border rounded-lg px-3 py-2 bg-[var(--card-surface)] text-sm" data-testid="quarantine-status-filter">
