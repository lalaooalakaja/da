...
const STATUS_CFG = {
  ...
  cancelled:   { label: 'Dibatalkan',      color: '#94a3b8', bg: 'bg-muted dark:bg-slate-400/20',   text: 'text-muted-foreground/60 dark:text-slate-400' },
};
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(req => <RequestCard key={req.id} req={req} onRefresh={load} />)}
        </div>
      )}
