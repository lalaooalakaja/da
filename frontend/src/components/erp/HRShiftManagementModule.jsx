      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {shift.is_default && <span className="text-[10px] text-muted-foreground/60 bg-muted px-1.5 py-0.5 rounded">default</span>}
          </div>
        ...
        {[{key:'shifts',label:'Template Shift'},{key:'assignments',label:'Assignment'}].map(t => (
          <button key={t.key} onClick={() => { setTab(t.key); setView('list'); }}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              tab === t.key ? 'bg-blue-500 text-foreground shadow-sm' : 'text-muted-foreground/50 hover:bg-muted'
            }`}>{t.label}</button>
        ))}
      </div>
                        ...
                        <tr key={a.id} className="hover:bg-muted">
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                              a.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground/60'
                            }`}>{a.status}</span>
                          </td>
                        </tr>
                      ))}
