...
def test_create_techpack_with_new_fields():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            results.check(data.get('size_columns') == ["STANDAR", "JUMBO"], "Size columns persisted")
...
def test_promote_to_production():
    ...
    try:
        ...
        # First, set style status to approved_for_launch
        print("    Setting style status to approved_for_launch...")
        r = requests.put(
            f"{BASE_URL}/dewi/rnd/styles/{test_data['style_id']}",
            headers=headers,
            json={"status": "approved_for_launch"},
            timeout=15
        )
        
        if r.status_code != 200:
            print(f"    ⚠️  Could not set status: {r.status_code}")
