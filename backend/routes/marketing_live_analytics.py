...
async def _sessions_in_range(db, start_iso: str, end_iso: str, platform: Optional[str] = None) -> list:
    ...
    pipeline = [
        ...
        {"$project": {
            ...
            # RC-15 (perluasan): field SSOT = gmv/total_orders/cr_rate —
            # total_revenue/orders_count/conversion_rate TIDAK ADA di koleksi
            "total_revenue": {"$ifNull": ["$gmv", 0]},
            "orders_count": {"$ifNull": ["$total_orders", 0]},
            "units_sold": {"$ifNull": ["$units_sold", 0]},
            "conversion_rate": {"$ifNull": ["$cr_rate", 0]},
