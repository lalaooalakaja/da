...
async def _ai_detect_schema(text_content: str, filename: str, file_type: str, is_image: bool = False) -> dict:
    """Deteksi skema via AI (structured output / ai_json terpusat). Returns mapping dict."""
    ...
    system_msg = ("You are a data analyst specializing in Indonesian marketplace data "
                  "(Shopee, TikTok, Tokopedia). Respond with a single valid JSON object only, no markdown.")
    ...
    try:
        ...
        result.setdefault("column_mappings", [])
        return result
    except AIError as e:
        logger.error(f"AI schema detection failed (AIError): {e}")
        return {"detected_source": "unknown", "platform": "unknown", "overall_confidence": 0.3,
                "warnings": [("AI budget terlampaui" if getattr(e, "over_budget", False) else f"AI gagal: {e}")],
                "column_mappings": []}
    except Exception as e:
        logger.error(f"AI schema detection failed: {e}")
        return {"detected_source": "unknown", "platform": "unknown", "overall_confidence": 0.3,
                "warnings": [f"Deteksi skema gagal: {e}"], "column_mappings": []}
...
async def _ai_normalize_rows(rows: list, column_mappings: list, source_type: str) -> list:
    ...
    from ai_cost_tracker import ai_json
    ...
    for batch_start in range(0, len(rows), batch_size):
        ...
        try:
            batch_results = await ai_json(
                feature="import-normalize-rows",
                system_message="Data normalization expert for Indonesian marketplace data. Respond only with a valid JSON array.",
                user_message=prompt,
                tier="standard",
                default=[],
            )
