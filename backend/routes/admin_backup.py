...
UPLOAD_TMP_DIR = Path("/app/backups/.uploads_tmp")
...
def _write_op_log(backup_id: str, kind: str, output: str) -> Optional[str]:
    ...
    try:
        ...
        path = target / f"{kind}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
async def upload_init(request: Request, body: UploadInitRequest):
    ...
    upload_id = f"up_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{secrets.token_hex(4)}"
...
async def clear_collections(request: Request, body: ClearCollectionsRequest):
    ...
    if body.create_backup:
        name = f"sebelum_kosongkan_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        try:
