...
async def _gen_settlement_number(db) -> str:
    """Nomor penyelesaian dinas — race-safe & formatnya bisa diatur owner
    (kunci `employee_travel_settlements.settlement_number`)."""
    ym = datetime.now().strftime('%Y%m')
    return await gen_prefixed_number(db, 'employee_travel_settlements', 'settlement_number',
                                     f'TS-{ym}-', 4)
