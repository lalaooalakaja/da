...
async def live_summary(request: Request):
    ...
    # Overall stats — RC-15: field SSOT = gmv/total_orders/peak_viewers/cr_rate
    # (revenue/orders/total_viewers/engagement_rate/conversion_rate TIDAK ADA di koleksi)
    pipeline = [
        {"$group": {
            "_id": None,
            ...
            "total_viewers": {"$sum": {"$ifNull": ["$peak_viewers", 0]}},
            "avg_conversion": {"$avg": "$cr_rate"}
        }}
    ]
    ...
    platform_pipeline = [
        {"$group": {
            "_id": "$platform",
            "sessions": {"$sum": 1},
            "revenue": {"$sum": {"$ifNull": ["$gmv", 0]}},
            "viewers": {"$sum": {"$ifNull": ["$peak_viewers", 0]}}
        }}
    ]
    by_platform = {}
    ...
    return success_response(data={
        ...
        # RC-15: guard None — engagement_rate tak punya SSOT (0 jujur); cr_rate bisa None
