...
async def _llm_column_mapping(headers: List[str], sample_rows: List[dict], system_fields: dict, key: str = None) -> dict:
    """Suggest column mapping via central AI wrapper (ai_json structured output).

    Returns {mapping, overall_confidence, notes}. Falls back to heuristic on any AI failure.
    `key` param dipertahankan untuk backward-compat (tidak dipakai; wrapper pakai EMERGENT_LLM_KEY).
    """
    import json as _json
    from ai_cost_tracker import ai_json, AIError
    ...
    valid_targets = " | ".join(list(system_fields.keys()) + ["null"])

    system_msg = (
        "Anda adalah AI mapper data penjualan e-commerce Indonesia (Shopee/TikTok/Tokopedia) ke ERP. "
        "Balas HANYA dengan satu objek JSON valid, tanpa prosa, tanpa markdown fences."
    )

    user_msg = f"""Petakan kolom sumber dari file upload ke field sistem ERP.
...
ATURAN MAPPING:
- Petakan setiap kolom sumber ke satu field sistem yang paling sesuai, atau null bila tidak relevan.
- target_field HARUS salah satu dari: {valid_targets}
...
- Berikan confidence 0-1 per kolom.
...
"""
    ...
    try:
        parsed = await ai_json(
            feature="marketing-import-mapping",
        ...
        mapping_by_source = {}
        for m in parsed.get('mapping', []) or []:
            if isinstance(m, dict) and m.get('source_column') is not None:
                mapping_by_source[str(m['source_column'])] = m

        normalized = []
        for h in headers:
            m = mapping_by_source.get(h)
            if m:
                tgt = m.get('target_field')
                if tgt in (None, "", "null", "None"):
                    tgt = None
                elif tgt not in valid_fields:
                    tgt = None
                try:
                    conf = float(m.get('confidence') or 0)
                except (TypeError, ValueError):
                    conf = 0.0
                normalized.append({"source_column": h, "target_field": tgt, "confidence": conf})
            else:
                normalized.append({"source_column": h, "target_field": None, "confidence": 0})

        try:
            overall = float(parsed.get('overall_confidence') or 0)
        except (TypeError, ValueError):
            overall = 0.0
        ...
        logger.warning(f"LLM column mapping AIError, fallback heuristic: {e}")
        return _heuristic_mapping(headers, system_fields)
    except Exception:
        logger.exception("LLM column mapping failed, fallback heuristic")
        return _heuristic_mapping(headers, system_fields)
...
def _convert_value(value, target_type: str):
    ...
    try:
        ...
        if target_type == 'number':
            # Locale-ID aware parsing (SSOT: utils.money.parse_id_number).
            # Fixes: "Rp 150.000"->150000, "1.234.567,89"->1234567.89, "150,5"->150.5
            from utils.money import parse_id_number, MoneyParseError
            try:
                return parse_id_number(s), None
            except MoneyParseError as me:
                return None, str(me)
        elif target_type == 'integer':
            from utils.money import parse_id_int, MoneyParseError
            try:
                return parse_id_int(s), None
            except MoneyParseError as me:
                return None, str(me)
