...
async def _generate_claim_number(db) -> str:
    """Nomor klaim biaya — race-safe & formatnya bisa diatur owner
    (kunci `rahaza_expense_claims.claim_number`)."""
    ym = datetime.now().strftime('%Y%m')
    return await gen_prefixed_number(db, 'rahaza_expense_claims', 'claim_number',
                                     f'EC-{ym}-', 4)
