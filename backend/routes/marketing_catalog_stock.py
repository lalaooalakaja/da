...
async def sync_stock_from_wms(catalog_id: str, request: Request):
    ...
    # ── Fase 3b: sinkron item yang tertaut VARIAN (variant_sku) tapi tanpa material_id ──
    # Jembatan by SKU: rahaza_model_variants.sku == kode FG (rahaza_materials type='fg').
    from routes.marketing_catalog_items import resolve_fg_stock_by_sku  # lazy import (hindari circular)
    variant_items = await db.marketing_catalog_items.find(
        {
            'catalog_id': catalog_id,
            'variant_sku': {'$exists': True, '$nin': [None, '']},
            '$or': [{'material_id': {'$in': [None, '']}}, {'material_id': {'$exists': False}}],
        },
        {'_id': 0},
    ).to_list(500)
    ...
    for item in variant_items:
        vsku = (item.get('variant_sku') or '').strip()
        if not vsku:
            continue
        fg = await resolve_fg_stock_by_sku(db, vsku)
        if not fg['found']:
            variant_not_found += 1
            continue
        new_qty = float(fg['available'])
        old_qty = float(item.get('stock_quantity', 0))
        ...
        await db.marketing_catalog_items.update_one(
            ...
            {'$set': {
                ...
                'stock_status': _stock_status(new_qty, threshold),
                'fg_onhand': fg['onhand'],
                'fg_reserved': fg['reserved'],
                'fg_available': fg['available'],
                'fg_material_id': item.get('fg_material_id') or fg.get('fg_material_id'),
                'stock_source': 'variant_sku',
                'updated_at': _now(),
                'last_stock_sync': _now(),
            }},
        )
        variant_logs.append({
            ...
            'sync_type': 'fg_variant_sync', 'notes': f'Sync FG by SKU varian: {vsku}',
            'synced_at': _now(), 'synced_by': user.get('id', ''),
        })
        variant_synced += 1

    if variant_logs:
        await db.marketing_stock_syncs.insert_many(variant_logs)

    await _refresh_catalog_stats(db, catalog_id)

    return {
        'ok': True,
        'synced': synced + variant_synced,
        'synced_material': synced,
        'synced_variant': variant_synced,
        'not_found_in_wms': not_found + variant_not_found,
        'total_linked': len(linked_items) + len(variant_items),
        'message': (
            f'Sinkron stok selesai: {synced + variant_synced} item diperbarui '
            f'({synced} via material, {variant_synced} via varian/SKU), '
            f'{not_found + variant_not_found} tidak ditemukan di WMS.'
        ),
