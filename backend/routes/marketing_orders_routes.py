...
async def seed_orders_if_empty():
    ...
    for i in range(60):
        ...
        # Fulfillment pipeline status (drives Gudang → Fulfillment queue)
        if status == "new":
            fulfillment_status = "pending_fulfillment"
        elif status == "packed":
            fulfillment_status = ["allocated", "picking", "packed_ready"][i % 3]
        ...
        orders.append({
            ...
            "allocations":    [],
            "dispatched_at":  (shipped_date.isoformat() if (fulfillment_status == "dispatched" and shipped_date) else None),
            "courier":        courier,
