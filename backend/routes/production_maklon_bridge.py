...
async def mature_ap_from_cmt_receipt(db, receipt_id: str, user: dict) -> dict:
    ...
    # Try to find "cmt_partner_id" mapping for compat with existing dewi_cmt_partners.
    partner = None
    if cmt_vendor_id:
    ...
    doc = {
        ...
        # Sekarang: `cmt_partner_id` = id master CMT bila ketemu, DAN `vendor_id`
        # selalu menyimpan id `vendor_partners` (SSOT vendor produksi/maklon),
        # sehingga kedua layar bisa mengelompokkan dengan benar.
        'cmt_partner_id': (partner or {}).get('id') or cmt_vendor_id or '',
        'vendor_id': cmt_vendor_id or '',
