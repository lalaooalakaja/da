...
async def _generate_trip_number(db) -> str:
    """Nomor perjalanan dinas — race-safe & formatnya bisa diatur owner
    (kunci `employee_travel_requests.trip_number`)."""
    ym = datetime.now().strftime('%Y%m')
    return await gen_prefixed_number(db, 'employee_travel_requests', 'trip_number',
                                     f'TR-{ym}-', 4)
